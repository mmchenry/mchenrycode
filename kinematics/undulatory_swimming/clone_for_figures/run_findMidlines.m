...
%                 'peak_data.mat']))
            ...
            % Directories
            %vid_dir = [vidRoot filesep dsVid(i).dir{j}];
%             vid_dir = ['/Users/mmchenry/Documents/MATLAB/shiner_video/' ...
%                           filesep ds(i).dir{j}];
            vid_dir = ['/Users/mmchenry/Documents/MATLAB/shiner_video/' ...
                        'ControlFish/Controlfish2_7_S0001'];
            ...
            for k = 1:size(xVals_f,2)
                ...
                if ~isempty(zros)
                    ...
                    pk(L).yMid(M,:)   = yVals(:,k)';
                    pk(L).sMid(M,:)   = s';
                    pk(L).fpath{M}    = [vid_dir filesep imFiles(k).name];
...
coefs1 = [d.amp_coef1.pre_slow(3,2) d.amp_coef2.pre_slow(3,2) ...
                     d.amp_coef3.pre_slow(3,2)];

coefs2 = [d.amp_coef1.post_slow(4,2) d.amp_coef2.post_slow(4,2) ...
                     d.amp_coef3.post_slow(4,2)];                 
                 clear d
...
for i = 1 % =  1:length(ds)
    ...
    for j = [1 4]%1:length(ds(i).dir)
        ...
        if ~isempty(ds(i).dir{j}) && ...
            ...
            % Load bodylength
            load([dataRoot filesep ds(i).dir{j} filesep 'bodyLength.mat'])
            bLen = bLength.pix;
            
            clear bLength
            
            % MIDLINES FOR PRE DATA
            
            meanY = mean(mean(mid2.yVals(:,:)./bLen));
            ...
            if j==1
                ...
                subplot(2,1,2)
                %for k = 225:5:275 %Values for i=5
                for k = 500:5:565 %Values for i=1
                    x = mid2.xVals(:,k)./bLen;
                    y = mid2.yVals(:,k)./bLen;
                    plot((x-x(1)),y-meanY,'k-')
                    hold on;
                    axis equal;
                    title(num2str(k))
                    %pause(.2);
                ...
                subplot(2,1,2)
                %for k =610:5:670 %Values for i=5
                for k = 250:5:310 %Values for i=1
                    x = mid2.xVals(:,k)./bLen;
                    y = mid2.yVals(:,k)./bLen;
                    plot((x-x(1)),y-meanY+.02,'k-')
                    hold on;
                    axis equal;
                     title(num2str(k))
                    %pause(.2);
...
%% Visualize wave peaks
if runVisualizeWaves
...
%vid_dir = '/Users/mmchenry/Documents/MATLAB/shiner_video/NeoFish/NeoFish1_1_S0001';
...
%             for n = 1:skipFrames:length(pk(k).fpath)
%                 subplot(numRows,1,m)
%                 im = imread(pk(k).fpath{n});
%                 warning off
%                 imshow(im)
%                 hold on
%                 warning on
%                 
%                 t = (pk(k).f(n) - pk(k).f(1))./250;
%                 
%                 plot(pk(k).xVid(n),pk(k).yVid(n),'.r')
%                 title(['t = ' num2str(t)]);
%                 
%                 if m==numRows
%                     figure;
...
%             end
            ...
            t = (pk(k).f - pk(k).f(1))./250;
            s = pk(k).s;
            c = polyfit(t,s,1);
            plot(t,s,'ko',t,polyval(c,t),'-k')
    end
...
if runAmp
   ...
   % Load d
    ...
    a1Pre = d.amp_coef1.pre_slow(:,2);
    a2Pre = d.amp_coef2.pre_slow(:,2);
    a3Pre = d.amp_coef3.pre_slow(:,2);
    
    % PRE
    for i = 1:length(a1Pre)
        yVals(i,:) = myfun([a1Pre(i) a2Pre(i) a3Pre(i)],s);
    end
  
    [mu,sigma,ci,sigmaI] = normfit(yVals);
...
%    h = fill([s' s(end:-1:1)'],[L1 L2(end:-1:1)],'r');
%     set(h,'MarkerFaceColor',clr_pre)
%     set(h,'MarkerEdgeColor','none')
    ...
    a1Post = d.amp_coef1.post_slow(:,2);
    a2Post = d.amp_coef2.post_slow(:,2);
    a3Post = d.amp_coef3.post_slow(:,2);
    
    for i = 1:length(a1Post)
        yVals(i,:) = myfun([a1Post(i) a2Post(i) a3Post(i)],s);
    end
...
%     h = fill([s' s(end:-1:1)'],[L1 L2(end:-1:1)],'g');
%     set(h,'MarkerFaceColor',clr_post)
%     set(h,'MarkerEdgeColor','none')
%     alpha(h,.5)
    set(gca,'TickLength',[.02 .02])
    axis([-.01 1 ylim])
...
if runStats
    ...
    s.exp     = [s.exp(1,:); s.exp(3:end,:)];  
    s.indiv   = [s.indiv(1); s.indiv(3:end)]; 
    s.spd     = [s.spd(1); s.spd(3:end)];
    s.val     = [s.val(1); s.val(3:end)];
    s.n       = [s.n(1); s.n(3:end)];
...
[mu,sigma,muI,sigmaI] = normfit([d.pre_slow(1,2);d.pre_slow(3:end,2)]);
