...
%% Scan frames for data preview   

% disp(' '); disp(' Running data preview . . .');
% 
% % Define frame numbers
% frames = p.start_frame:fSkip:(min([length(p.files) p.end_frame]));
% 
...
%     
%     % Find pixel values
%     [val_x,val_p] = improfile_horz(im,p.xLine,p.yLine,num_pix);
%     
%     % Find approximate pipette tip x-position
%     val_p_2nd  = val_p(round(length(val_p)/2):end);
%     tip_idx = find(val_p == min(val_p_2nd),1,'last');
%     
%     % Store data
%     vals_p(i,:) = val_p';
%     tip_x(i,1)  = val_x(tip_idx);
%     
%     clear im vals cFrame val_p_2nd tip_idx val_p
%     
% end
% 
% disp('                        . . . done');
% 
% clear i im cFrame val_x val_p
