...
thetaOut = linspace(0,pi,numVals);

% Calculate length between B & D for range of theta
h_BD     = sqrt(p.L1^2 + p.L2^2 - 2.*p.L1.*p.L2.*cos(thetaOut));
...
thetaOut = thetaOut(idx);
h_BD     = h_BD(idx);

clear idx

% Calculate possible thetaIn values
thetaIn  = acos( (p.L2.^2 + p.L1.^2 - h_BD.^2) ./ (2.*p.L1.*p.L2) );
