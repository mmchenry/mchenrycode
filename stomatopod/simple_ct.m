...
% CT analysis (that does not include an interactive component) to calculate 
% the moment of inertia.
%
% Use pool_dactyl.m to analyze resulting data.
...
if isempty(a)
    ...
    im = imread([imPath filesep fileNames{s.frLatJoint}]);
    if size(im,3)==3
        im = rgb2gray(im);
    end
    f = figure;
...
if isempty(a)
    ...
    for i = 1:length(sliceList) 
        ...
        % Product of radius^2 and pixel intensity for slice
        prod1 = r.^2 .* double(im);
        
        % Pixel intensity for slice
        pixInt = double(im);
        
        % distance between image center and joint
        im_dist = ( (max(X(:))/2-Jx).^2 + (max(Y(:))/2-Jy).^2 + (Z(i)-Jz).^2 ).^0.5;
        
        % Store data in 'd'
        d.image_dist(i,1) = im_dist;
        d.pixr2(i,1)      = sum(prod1(:));
        d.zVal(i,1)       = Z(i);
        d.pixIntensity(i,1) = sum(pixInt(:));
        ...
        % Clear variables
        clear im sNum se T Tremain prod1 im_dist
    end
